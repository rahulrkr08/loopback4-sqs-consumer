...
import {SqsConsumerConfig, SqsConsumerOptions} from './types';
...
export class SqsConsumer {
  ...
  constructor(
    ...
    const queueBindings: Readonly<Binding<any>>[] = this.app.findByTag({
  ...
  private createPollers(queueBindings: Readonly<Binding<any>>[] = []): Promise<Consumer[]> {
    const {sqs} = this.sqsConsumerConfig

    return Promise.all(queueBindings.map(async (queueBinding: Readonly<Binding>) => {
      const consumerOptions = queueBinding.tagMap[SqsConsumerOptionsKeyBindings.OPTIONS];
      const handlerFn: Function | undefined = await this.app.get(queueBinding.key);
      const handler = (message: any) => handlerFn && handlerFn({message});
      const handlerKey = consumerOptions.batch ? 'handleMessageBatch' : 'handleMessage';
